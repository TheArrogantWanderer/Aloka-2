...
function AISummarySection({article, onGenerate}) {
  ...
  return (
    <div onClick={handleToggle} style={{background:"rgba(245,166,35,0.07)",border:"1px solid rgba(245,166,35,0.2)",borderRadius:12,padding:"12px 14px",cursor:"pointer",marginBottom:16,transition:"background 0.2s"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <span style={{color:"#F5A623",fontSize:14,transition:"transform 0.2s",display:"inline-block",transform:open?"rotate(45deg)":"none"}}>+</span>
      </div>
      {open&&(
...
// ── News Card ──────────────────────────────────────────────────────────────
function NewsCard({article, onSkip, onSave, isTop, stackIndex, onGenerateAI}) {
  ...
  const handleEnd=()=>{if(!dragging||!isTop)return;setDragging(false);if(Math.abs(currentX.current)>100){currentX.current>0?onSave(article):onSkip(article);}else setOffset(0);currentX.current=0;};
  const rotation=isTop?offset*0.07:0;
  const swipeDir=offset>60?"save":offset<-60?"skip":null;
  const stackStyles={0:{transform:`translateX(${offset}px) rotate(${rotation}deg)`,zIndex:10,opacity:Math.max(0.3,1-Math.abs(offset)/300)},1:{transform:"translateY(10px) scale(0.97)",zIndex:9,opacity:0.85},2:{transform:"translateY(20px) scale(0.94)",zIndex:8,opacity:0.6}};
  return (
    <div onMouseDown={e=>handleStart(e.clientX)} onMouseMove={e=>handleMove(e.clientX)} onMouseUp={handleEnd} onMouseLeave={handleEnd}
      ...
      style={{position:"absolute",top:0,left:0,right:0,background:"#1A1A1A",borderRadius:20,padding:24,cursor:isTop?"grab":"default",userSelect:"none",transition:dragging?"none":"transform 0.35s cubic-bezier(0.34,1.56,0.64,1), opacity 0.2s ease",border:"1px solid #2A2A2A",boxShadow:isTop?"0 20px 60px rgba(0,0,0,0.6)":"none",overflowY:isTop?"auto":"hidden",maxHeight:560,...(stackStyles[stackIndex]||{display:"none"})}}>
      {isTop&&swipeDir==="save"&&<div style={{position:"absolute",top:20,left:20,background:"#F5A623",color:"#000",padding:"4px 12px",borderRadius:20,fontSize:12,fontWeight:700,opacity:Math.min(1,(offset-60)/60),zIndex:20}}>SAVED ✓</div>}
      {isTop&&swipeDir==="skip"&&<div style={{position:"absolute",top:20,right:20,background:"#2A2A2A",color:"#666",padding:"4px 12px",borderRadius:20,fontSize:12,fontWeight:700,opacity:Math.min(1,(-offset-60)/60),zIndex:20}}>SKIP</div>}
      ...
      {isTop&&(
        <div style={{display:"flex",gap:10}}>
          ...
          <a href={article.url} target="_blank" rel="noopener noreferrer" style={{flex:2,padding:11,background:"#1A1A1A",border:"1px solid #2A2A2A",borderRadius:12,color:"#CCC",fontSize:13,fontWeight:600,textDecoration:"none",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',sans-serif"}}>Read full article →</a>
          <button onClick={()=>onSave(article)} style={{flex:1,padding:11,background:"#F5A623",border:"none",borderRadius:12,color:"#000",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>Save</button>
        </div>
      )}
    </div>
  );
...
export default function AlokaApp() {
      ...
      if(data.status==="ok"&&data.news?.length>0){
        const formatted=data.news
          ...
          .map(a=>({
            ...
            bias:getBias(a.author),
            time:a.published?timeAgo(a.published):"Recently",
  ...
  const handleSave=(article)=>{setSaved(prev=>prev.find(a=>a.id===article.id)?prev:[...prev,article]);setArticles(prev=>prev.filter(a=>a.id!==article.id));showToast("Saved ✓");};
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0D0D0D",fontFamily:"'Inter',-apple-system,sans-serif",color:"#fff",display:"flex",flexDirection:"column",maxWidth:420,margin:"0 auto",position:"relative",fontSize:tsz}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              ...
              <span style={{fontSize:11,color:"#555"}}>{loading?"Loading…":isLive?"Live":"Demo mode"}</span>
            </div>
      ...
      {activeTab==="home"&&<>
        ...
        <div style={{flex:1,padding:"20px 20px 110px"}}>
          <div style={{display:"flex",justifyContent:"space-between",padding:"0 8px",marginBottom:10}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:5}}><span style={{fontSize:11,color:"#444"}}>Save</span><span style={{fontSize:15}}>→</span></div>
          </div>
          {loading?<div style={{position:"relative",height:500}}><SkeletonCard/></div>
          :articles.length>0?<div style={{position:"relative",height:580}}>{articles.slice(0,3).map((article,i)=><NewsCard key={article.id} article={article} isTop={i===0} stackIndex={i} onSkip={handleSkip} onSave={handleSave} onGenerateAI={handleGenerateAI}/>)}</div>
      ...
      {activeTab==="saved"&&<div style={{flex:1,padding:"20px 20px 100px",overflowY:"auto"}}>
        ...
        {saved.length===0?<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:300,gap:12}}><div style={{fontSize:40}}>🔖</div><p style={{color:"#444",fontSize:14,textAlign:"center"}}>Swipe right on any story to save it here.</p></div>
        :saved.map(article=>(
          <div key={article.id} style={{background:"#1A1A1A",borderRadius:14,padding:16,border:"1px solid #2A2A2A",marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10}}>{displayPrefs.showBiasLabels&&<BiasTag bias={article.bias}/>}<button onClick={()=>handleRemoveSaved(article.id)} style={{background:"none",border:"none",color:"#444",cursor:"pointer",fontSize:16,padding:0}}>×</button></div>
            </div>
      ...
      {activeTab==="settings"&&<div style={{flex:1,padding:"20px 20px 100px",overflowY:"auto"}}>
        ...
        <div style={{marginBottom:28}}>
          ...
          {user?(
            <div style={{background:"#1A1A1A",borderRadius:14,padding:20,border:"1px solid #222"}}>
              ...
              <p style={{fontSize:12,color:"#444",margin:"0 0 14px"}}>✓ Preferences syncing across devices</p>
        ...
        <div style={{marginBottom:28}}>
          ...
          <p style={{fontSize:12,color:"#555",marginBottom:14}}>Choose which categories appear in your All feed.</p>
          {CATEGORIES.filter(c=>c!=="All").map(cat=>(
        ...
        <div style={{marginBottom:12}}>
          ...
          <div style={{background:"#1A1A1A",borderRadius:14,padding:20,border:"1px solid #222"}}>
            {[["App","Aloka"],["Version","0.4.0 Beta"],["Tagline","News, clearly."],["Name origin","Sinhala — light"]].map(([k,v])=>(
