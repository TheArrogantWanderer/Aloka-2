...
function SwipeTutorial({ onDone }) {
  return (
    <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.92)",zIndex:50,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",borderRadius:20,padding:24}}>
      <div style={{fontSize:40,marginBottom:16}}>👋</div>
      <p style={{fontSize:17,fontWeight:700,color:"#fff",textAlign:"center",marginBottom:24,fontFamily:"'Playfair Display',Georgia,serif"}}>Here's how Aloka works</p>
      <div style={{display:"flex",flexDirection:"column",gap:16,marginBottom:28,width:"100%"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14,background:"#1A1A1A",borderRadius:14,padding:"14px 16px"}}>
          <span style={{fontSize:24}}>👉</span>
          <div><p style={{fontSize:14,fontWeight:700,color:"#fff",margin:"0 0 2px"}}>Swipe right to read</p><p style={{fontSize:12,color:"#555",margin:0}}>Opens the full article.</p></div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:14,background:"#1A1A1A",borderRadius:14,padding:"14px 16px"}}>
          <span style={{fontSize:24}}>🔖</span>
      ...
      <button onClick={onDone} style={{width:"100%",padding:14,background:"#F5A623",border:"none",borderRadius:12,color:"#000",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>Got it — show me the news</button>
    </div>
  );
}
...
// ── News Card ──────────────────────────────────────────────────────────────
function NewsCard({article, onSkip, onRead, onSave, isTop, stackIndex, onGenerateAI, showTutorial}) {
  const startX=useRef(null), currentX=useRef(0);
  const [dragging,setDragging]=useState(false);
  const [offset,setOffset]=useState(0);
  const [tutorialDone,setTutorialDone]=useState(()=>LS.get("aloka_tutorialdone",false));
  ...
  const handleEnd=()=>{
    ...
    if(Math.abs(currentX.current)>100){
      if(currentX.current>0){ window.open(article.url,"_blank"); onRead(article); }
      else onSkip(article);
    } else setOffset(0);
    currentX.current=0;
  };

  const handleTutorialDone=()=>{ LS.set("aloka_tutorialdone",true); setTutorialDone(true); };
  ...
  return (
    <div onMouseDown={e=>handleStart(e.clientX)} onMouseMove={e=>handleMove(e.clientX)} onMouseUp={handleEnd} onMouseLeave={handleEnd}
      ...
      {isTop&&!tutorialDone&&<SwipeTutorial onDone={handleTutorialDone}/>}
      ...
      {isTop&&(
        <div style={{display:"flex",gap:10}}>
          ...
          <a href={article.url} target="_blank" rel="noopener noreferrer" onClick={()=>onRead(article)} style={{flex:2,padding:11,background:"#F5A623",border:"none",borderRadius:12,color:"#000",fontSize:13,fontWeight:700,textDecoration:"none",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',sans-serif"}}>Read →</a>
...
export default function AlokaApp() {
  ...
  const handleRead=(article)=>{setArticles(prev=>prev.filter(a=>a.id!==article.id));};
